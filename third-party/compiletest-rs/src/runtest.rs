...
use common::{Assembly, Btf, Incremental, MirOpt, RunMake, Ui};
...
\n",
    ...
    fn compose_and_run_compiler(&self, mut rustc: Command, input: Option<String>) -> ProcRes {
        ...
        for rel_ab in &self.props.aux_builds {
            ...
            let mut aux_rustc =
                aux_cx.make_compile_args(&aux_testpaths.file, aux_output, AllowUnused::No);
    ...
    fn make_compile_args(
        ...
        match self.config.mode {
            ...
            RunFail | RunPassValgrind | Pretty | DebugInfoGdb | DebugInfoLldb | Codegen
            | Rustdoc | RunMake | CodegenUnits | Assembly | Btf => {
    ...
    fn compile_btf_test_and_save_object(&self) -> (ProcRes, PathBuf) {
        // This works with both `--emit asm` (as default output name for the assembly)
        // and `ptx-linker` because the latter can write output at requested location.
    ...
    fn btf_dump(&self, output: &Path) -> (ProcRes, PathBuf) {
        let output_path = self.output_base_name().with_extension("btf.txt");
        let mut bpftool = Command::new(self.config.bpftool.as_ref().unwrap());
        bpftool.arg("btf").arg("dump").arg("file").arg(output);

        let pr = self.compose_and_run(bpftool, "", None, None);
        if pr.status.success() {
            let mut f = File::create(output_path.clone()).unwrap();
            f.write_all(pr.stdout.as_bytes()).unwrap();
        }
        (pr, output_path)
    }
    ...
    fn run_btf_test(&self) {
        if self.config.llvm_filecheck.is_none() {
            self.fatal("missing --llvm-filecheck");
        }

        if self.config.bpftool.is_none() {
            self.fatal("missing --bpftool");
        }

        // we need to change the props of the test context to remove
        // -C "prefer-dynamic" compile option (preventing to use -C lto=true)
        let mut props = self.props.clone();
        props.no_prefer_dynamic = true;
        let test_cx = TestCx {
            props: &props,
            ..*self
        };

        let (proc_res, output_path) = test_cx.compile_btf_test_and_save_object();
        if !proc_res.status.success() {
            test_cx.fatal_proc_rec("compilation failed!", &proc_res);
        }

        let (proc_res, output_path) = test_cx.btf_dump(&output_path);
        if !proc_res.status.success() {
            test_cx.fatal_proc_rec("dumping BTF failed", &proc_res);
        }

        let proc_res = test_cx.verify_with_filecheck(&output_path);
        if !proc_res.status.success() {
            test_cx.fatal_proc_rec("verification with 'FileCheck' failed", &proc_res);
        }
    }
    ...
    fn normalize_output(&self, output: &str, custom_rules: &[(String, String)]) -> String {
        ...
        let json =
            cflags.contains("--error-format json") || cflags.contains("--error-format pretty-json");
